...
from src.osint_sources.google import google
...
def getArguments(args):
    ...
    parser = argparse.ArgumentParser(
        description='EI - This tool scrapping social media to get information from a target')
    parser.add_argument('-t', '--tag', dest='tag',
                        help='Insert the option to scrapper, options: tinder, twitter, google, facebook, instagram or all')
    ...
    parser.add_argument("-e", '--explicit', dest='explicit',
                        help="Default True. If true it search the exact text, if false it can search each word separately")
    ...
    if args.tag.lower() == "yandex":
        if not args.image:
            ...
            parser.print_help()
            sys.exit(-1)
        if not re.findall('http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+] |[!*\(\), ]|(?:%[0-9a-fA-F][0-9a-fA-F]))+',
                          args.image) and not args.token:
            print("--------------")
