...
def google(toSearch, placeToSearch, knownImage, number, verbose):
    ...
    if placeToSearch != None and len(placeToSearch) > 0:
        driver.get(
            "https://www.google.com/search?q=site:" + placeToSearch + "+AND+%22" + toSearch + "%22&source=lnms&tbm=isch&sa=X&ved=0ahUKEwiz2eSN_9vgAhUJoRQKHU8YCuwQ_AUIDigB&biw=1181&bih=902")
    else:
        driver.get(
            "https://www.google.com/search?q=" + toSearch + "&source=lnms&tbm=isch&sa=X&ved=0ahUKEwiz2eSN_9vgAhUJoRQKHU8YCuwQ_AUIDigB&biw=1181&bih=902")
            ...
            for actual_image in actual_images:
                if actual_image.get_attribute('src') and 'https' in actual_image.get_attribute('src'):
                    ...
                    if placeToSearch != None:
                        name = os.path.join('data/google/' + str(now) + '_images',
                                            str(i) + "-" + placeToSearch + "-" + toSearch + ".jpg")
                    else:
