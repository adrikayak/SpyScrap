...
def instagram(name_to_search, knownimage, verbose):
    ...
    for u in users:
        ...
        if knownimage:
            ...
            user = {'username': u['user']['username'], 'full_name': u['user']['full_name'],
                    'profile': 'https://www.instagram.com/' + u['user']['username'],
                    'is_private': u['user']['is_private'], 'is_verified': u['user']['is_verified'], 'image': image_name}
            j = j + 1
        else:
            user = {'username': u['user']['username'], 'full_name': u['user']['full_name'],
                    'profile': 'https://www.instagram.com/' + u['user']['username'],
                    'is_private': u['user']['is_private'], 'is_verified': u['user']['is_verified']}
